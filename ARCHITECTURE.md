...
| 3d | Prototype v10 (round 15, current): Ecosystem mascot module beside the drop — Plant / Polar bear / Glacier as original layered SVG with a `MASCOT_ASSETS` slot for licensed WebP frames; connected catch-up degradation (floe → 60 %, glacier wall → 55 %); plain-English `[i]` explainers replacing jargon on the dashboard; live mascot preview in the simulator; mascot state in the widget, share card and the new blue-glow "Add Widget" card | `sipcount.html` + `engine.js` |
| 3c | Prototype v9 (rounds 3–14): one page + one shared engine. Dashboard = draining water drop, opportunity cost, dynamic plant, insights, 7-day chart, all-time impact, tiles, scope split; input/output token model (output ≈5× input); water facts; weekly digital-sobriety report; widget previews; 30-min session check-ins; neutralize + share; Grove mock. **No beverage metaphors** (round 13/14) and **no Aqua branch** (round 14) | `sipcount.html` + `engine.js` → live link |
